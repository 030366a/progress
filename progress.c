...
bool flag_throughput = false;
bool flag_monitor = false;
bool flag_monitor_continuous = false;
...
bool is_ignored_file(char *str) {
    int i;
    for (i = 0; i < ignore_file_list_cnt; i++) {
        if (strcmp(ignore_file_list[i], str) == 0) {
            return true;
        }
    }
    return false;
...
int find_pids_by_binary_name(char *bin_name, pidinfo_t *pid_list, int max_pids) {
    ...
    while (true) {
        ...
        if (S_ISDIR(stat_buf.st_mode) && is_numeric(direntp->d_name)) {
            ...
            if (strcmp(basename(exe), bin_name) == 0) {
...
bool get_fdinfo(pid_t pid, int fdnum, fdinfo_t *fd_info) {
    struct stat stat_buf;
...
void copy_and_clean_results(result_t *results, int result_count, bool copy) {
    static result_t old_results[MAX_RESULTS];
    static int old_result_count = 0;

    if (copy == true) {
        for (int i = 0; i < old_result_count; ++i) {
            bool found = false;
...
int monitor_processes(int *nb_pid) {
    ...
    if (proc_specifiq_name_cnt) {
        ...
        for (i = 0; i < proc_specifiq_name_cnt; ++i) {
            pid_count += find_pids_by_binary_name(proc_specifiq_name[i], pidinfo_list + pid_count, MAX_PIDS - pid_count);
            if (pid_count >= MAX_PIDS) {
    ...
    if (proc_specifiq_pid) {
        ...
        for (i = 0; i < proc_specifiq_pid_cnt; ++i) {
            pid_count += find_pid_by_id(proc_specifiq_pid[i], pidinfo_list + pid_count);
    ...
    if (search_all == true) {
        for (i = 0; i < proc_names_cnt; i++) {
            pid_count += find_pids_by_binary_name(proc_names[i], pidinfo_list + pid_count, MAX_PIDS - pid_count);
            if (pid_count >= MAX_PIDS) {
                nfprintf(stderr, "Found too much procs (max = %d)\n", MAX_PIDS);
                return 0;
            }
    ...
    *nb_pid = pid_count;
    ...
    for (i = 0; i < pid_count; i++) {
        ...
        for (j = 0; j < fd_count; j++) {
            ...
            if (flag_open_mode && flag_open_mode != fdinfo.mode) continue;
    ...
    if (flag_throughput && !first_pass) usleep(1000000 * throughput_wait_secs);
    if (flag_monitor || flag_monitor_continuous) {
        clear();
    }
    copy_and_clean_results(results, result_count, true);
    for (i = 0; i < result_count; i++) {
        if (flag_throughput && !first_pass) {
            still_there = get_fdinfo(results[i].pid.pid, results[i].fd.num, &fdinfo);
            if (still_there && strcmp(results[i].fd.name, fdinfo.name)) still_there = false;  // still there, but it's not the same file !
        } else
            still_there = false;
        ...
        nprintf("[%5d] %s %s\n\t%.1f%% (%s / %s)", results[i].pid.pid, results[i].pid.name, results[i].fd.name, perc, fpos, fsize);
        ...
        if (flag_throughput && still_there && !first_pass) {
            ...
            usec_diff = (fdinfo.tv.tv_sec - results[i].fd.tv.tv_sec) * 1000000L + (fdinfo.tv.tv_usec - results[i].fd.tv.tv_usec);
    ...
    if (flag_monitor || flag_monitor_continuous) {
        if (!result_count) nprintf("No PID(s) currently monitored\n");
        refresh();
    }
    copy_and_clean_results(results, result_count, false);
    first_pass = false;
    return 0;
}
...
int main(int argc, char *argv[]) {
    ...
    if (env_progress_args) {
        ...
        int full_len = strlen(argv[0]) + 1 + strlen(env_progress_args) + 1;
    ...
    if (flag_monitor || flag_monitor_continuous) {
        ...
        mainwin = initscr() if (mainwin == NULL) {
            fprintf(stderr, "Error initialising ncurses.\n");
            exit(EXIT_FAILURE);
        }

        if (flag_throughput == false) {
            flag_throughput = true;
